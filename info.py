import streamlit as st

st.markdown(
    "<h1 style='text-align: center; font-size: 4em; margin-bottom: 0; padding-bottom: 0;'>Acerca de</h1>", unsafe_allow_html=True
    )

st.markdown(
    "<p <h1 style='text-align: justify; font-size: 1.2em; margin-top: 1.2rem; padding-bottom: 0;'>Somos una Institución de Educación Superior que forma profesionales de calidad, con actitud científica, creativa, solidaria, con espíritu emprendedor e innovador, sensibles a la diversidad cultural y comprometidos con el respeto a la valoración de la diferentes culturas formando profesionistas acorde a las demandas de la sociedad y desarrollando actividades académicas, de investigación, difusión y extensión universitaria, para beneficiar a la Región, el Estado y el País, con la razón de mejorar el desarrollo humano en los nuevos retos de nuestra era.</p>",
    unsafe_allow_html=True
    )

st.image("images/fachadaUICh.png")

col1, col2 = st.columns(2, gap="small", vertical_alignment='center')
with col1:
    st.image("images/fachadaiz.png")
with col2:
    st.title("CONOCENOS :)")
    st.write("15 AÑOS FORMANDO PROFESIONALES")
    st.write("La educación es la semilla de la prosperidad, en la UICh la sembramos cada día")